...
const SIGNER_URL = "/api/v1/assets/upload/signer-url";
...
/**
 * Method to calculate MD5 checksum for the given data
 * @param data
 */
const cryptoMd5Method = (data: unknown) => {
  let myWorker;
  if (window.Worker) {
    myWorker = new window.Worker("/PicoWorker.js");
  }
  return new Promise((resolve, reject) => {
    if (myWorker) {
      ...
      myWorker.onmessage = result => {
        myWorker.terminate();
        myWorker = null;
        resolve(result.data.hashData);
      };
    } else {
      var md5_digest = AWS.util.crypto.md5(data, "base64");
      resolve(md5_digest);
    }
  });
};
/**
 * Method to calculate cryptoHexEncodedHash256 for given data
 * @param data
 */
const cryptoHexEncodedHash256 = (data: unknown) => {
  return AWS.util.crypto.sha256(data, "hex");
};
/**
 * Method to get evaporate config params for S3 upload
 * @param   awsConfig
 * @returns config
 */
const getEvaporateConfig = (awsConfig: UploadAWSConfig) => {
  const config = {
    signerUrl: customAuth ? undefined : SIGNER_URL,
    customAuthMethod: API.customAuthMethodForSignerURL,
    aws_key: awsConfig.getAwsAccessKeyId(),
    bucket: awsConfig.getAwsBucketName(),
    awsRegion: awsConfig.getAwsRegion(),
    awsSignatureVersion: "4",
    computeContentMd5: true,
    maxConcurrentParts: 2,
    cryptoMd5Method: cryptoMd5Method,
    cryptoHexEncodedHash256: cryptoHexEncodedHash256,
    logging: false,
...
/**
 * Method to create evaporate object
 * @param fileUploaderData
 */
export const CreateEvaporateObj = awsConfig => {
  return new Promise(function (resolve, reject) {
    Evaporate.create(getEvaporateConfig(awsConfig))
      .then(evaporate => {
        resolve(evaporate);
      })
      .catch(error => {
...
export const cancelUploading = filesMap => {
  return new Promise((resolve, reject) => {
    let cancelPromises = [];
    console.log("cancel upload map size: ", filesMap.size);
    for (const [key, value] of filesMap.entries()) {
      console.log(`cancel upload for: ${key} , ${value.toString()}`);
      let cancelPromiseObject = Promise.resolve(value.cancelPromise()).then(_.noop).catch(_.noop);
      cancelPromises.push(cancelPromiseObject);
    }
    console.log("cancel upload promise array size: ", cancelPromises.length);
    Promise.all(cancelPromises).then(
      result => {
        console.log("All files upload cancelled successfully");
        resolve();
      },
      err => {
        console.log("All files cancel upload error with error  ", err.toString());
        reject();
      }
    );
  });
...
/**
 * Method to cancel all the uploads
 * @param evaporate
 */
export const cancelAllUpload = evaporate => {
  return new Promise((resolve, reject) => {
    /* for (let i = 0; i < storagePromises.length; i++){
      try {
        let cancelResult = Storage.cancel(storagePromises[i]);
        console.log("upload cancel for index " + i + "cancel result: " + cancelResult);
      } catch (err) {
        console.log("upload cancel error " + err.toString());
        if (reject)
          reject(err);
        continue;
      }
...
export const cancelUpload = (evaporate, id) => {
  if (evaporate && id) {
    evaporate.cancel(id);
  } else if (id) {
    Storage.cancel(id);
...
export const pauseUpload = (evaporate, id) => {
  if (evaporate && id) {
    evaporate.pause(id);
  }
};

export const pauseAllUpload = evaporate => {
  if (evaporate) {
    evaporate.pause();
  }
};

export const resumeAllUpload = evaporate => {
  if (evaporate) {
    evaporate.resume();
  }
};

export const resumeUpload = (evaporate, id) => {
  if (evaporate && id) {
    evaporate.resume(id);
  }
};
