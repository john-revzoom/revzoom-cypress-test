...
export default class AuthenticationController {
   ...
   * @param lastName
   */
  static async signUp(email: string, password: string, firstName: string, lastName: string): Promise<ISignUpResult> {
   ...
   * @returns
   */
  static async signIn(username: string, password: string): Promise<CognitoUserSession> {
    return await AuthenticationController.cognitoSignIn(username, password);
  }
   ...
   * @param password
   */
  static async cognitoSignIn(username: string, password: string): Promise<CognitoUserSession> {
  ...
  static async checkAndGetValidSession(user: CognitoUser, session: CognitoUserSession): Promise<CognitoUserSession> {
    ...
    if (!tenantIdExists) {
      ...
      console.log("api call result is : ", resp);
      if (resp.isPresent()) {
        let tenantId = resp.get().getTenantId();
        if (tenantId && tenantId.length > 0) {
          let attributeUpdatedResult = await AuthenticationController.updateAttributes(
            user,
            COGNITO_TENANT_ID_ATTRIBUTE_NAME,
            tenantId
          );
          console.log("Attribute update result is : ", attributeUpdatedResult);
      ...
      console.log("New session not needed. Session is : ", session);
      return session;
    }
  }
   ...
   * @returns
   */
  static updateAttributes(user: CognitoUser, attributeName: string, attributeValue: string): Promise<string> {
