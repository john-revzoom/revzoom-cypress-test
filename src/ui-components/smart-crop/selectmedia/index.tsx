...
const UploadMediaHeader = ({
  ...
  return (
    <div className={styles.uploadMedia_Header}>
      <Button onClick={onCloseClick} type="text" icon={<Image src="/images/cross.svg" width={10} height={10} />} />
      <div>
  ...
  implements UploadReporter, ThumbReporter
{
  ...
  onNext({ original, customFormats, socialMedia, parameters }: OBJECT_TYPE): void {
    ...
    try {
      API.startSmartCropAutomation(this.props.automationId, jobStartMessage).then(
        (response: Optional<JobStartResponse>) => {
