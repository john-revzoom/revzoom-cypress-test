import { FILE_NAME_DELIMITER, BACKWARD_SLASH, COMMA_DELIMITER, DEFAULT_ARRAY_JOIN_DELIMITER } from "./Constants";
...
export class StringUtil {
   ...
   */
  static toCamelCase(str: string): string | null {
   ...
   * @returns
   */
  static getFolderFromFileName(fileName: string): string | null {
    if (StringUtil.isNotEmpty(fileName)) {
      fileName = fileName.replace(/\//g, BACKWARD_SLASH);
      let lastIndex = fileName ? fileName.lastIndexOf(BACKWARD_SLASH) : -1;
      return lastIndex >= 0 ? fileName.substr(0, lastIndex) : null;
    } else {
      return null;
    }
  }
  ...
  static replaceExtension(fileName: string, newExtension: string): string {
    ...
    if (oldExt) {
      fileName = fileName.replace(oldExt, newExtension);
    }
    return fileName;
  }
   ...
   * @returns
   */
  static getFileNameFromPath(filePath: string): string | null {
   ...
   * @returns
   */
  static getParentDirFromFilePath(filePath: string): string | null {
    if (StringUtil.isNotEmpty(filePath)) {
   ...
   * @param delimiter
   */
  static joinTwoStrings(str1: string, str2: string, delimiter = ""): string | null {
    if (StringUtil.isNotEmpty(str1) && StringUtil.isNotEmpty(str2)) {
