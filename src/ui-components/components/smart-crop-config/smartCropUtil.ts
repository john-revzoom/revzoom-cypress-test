...
const generateConfigName = (cropType: string, cropSide: string, marker: string, position: number) => {
  const type = cropType.toLowerCase().replace("_", " ");
  const side = cropSide.toLowerCase();
...
export const createSaveConfigPayload = (cropInfo: any) => {
  ...
  configuration.marker = cropInfo?.cropMarker; //TODO: use value for marker; i.e., nose, lips, eyes, face
...
export const editConfigPayload = (cropInfo: any, id: string) => {
  //CropSideType
  const cropSideType = new CropSideType();
  cropSideType.top = cropInfo?.cropSize?.top;
  cropSideType.left = cropInfo?.cropSize?.left;
