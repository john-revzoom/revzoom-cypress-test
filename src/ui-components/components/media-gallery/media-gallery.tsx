...
const MediaGallery = ({
  ...
  const selectAll = () => {
    ...
    onSelectionChanged(images.map(image => image.id));
    if (onIgnoredForDelete) onIgnoredForDelete([]);
  };

  const selectVisibleImgs = () => {
    onSelectionChanged(renderedCards.map(image => image.id));
  };
  ...
  //       const columns = Math.floor(galleryRef.current.clientWidth / (width + 32));
  //       const rows = Math.floor(galleryRef.current.clientHeight / (height + 24)) + 1;
  //       setCardNumPerPage(rows * columns);
  //     }
  //   },
  //   [galleryRef]
  // );
  ...
  const galleryScrollListener = useCallback(
    ...
    [images, searchText, nextPageLoading]
  );
      ...
      <div className={styles.Header}>
        ...
        <HLayout noPadding={true} noFlex={true} gap={24}>
          ...
          <div>
            <input
              type="file"
              ref={viewMediaFileUploaderRef}
              onChange={onFileChanged}
              multiple
              accept={acceptUpload}
              className={styles.FileUploadButton}
              data-cy="viewMediaUpload"
            />
            <Tooltip title={t("upload.upload_more_tooltip_text")} placement="top">
              <div className={styles.uploadMoreBtn} onClick={() => viewMediaFileUploaderRef.current?.click()}>
                <img src={"/images/upload@1x.svg"}></img>
        ...
        <div ref={galleryRef} className={classNames(styles.CardList, className)} onScroll={galleryScrollListener}>
          <Row style={{}} gutter={[32, 24]}>
            {renderedCards.map((image, idx) => (
              <Col key={idx} xs={24} sm={12} md={8} lg={6} xl={6} xxl={4}>
